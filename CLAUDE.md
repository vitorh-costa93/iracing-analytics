...
- `laps.telemetry_skip` (11/09/2026, "zere esse backlog de telemetrias para só começar a puxar as novas daqui pra frente"): every lap that was already missing telemetry at that moment was marked `telemetry_skip=true` in a one-time migration; `lib/telemetry-backfill.ts`'s `pendingTelemetry()` excludes it, so the backfill only chases telemetry for laps that show up from that point on (a fresh sync insert never sets it, defaulting false). Non-destructive and reversible per-lap; do not mass-reset it back to false without the user's explicit request -- that would reopen the exact multi-thousand-lap backlog this was meant to close.
