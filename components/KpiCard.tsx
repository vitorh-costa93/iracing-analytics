type Props = { eyebrow: string; value: number | null; previousValue: number | null; previousLabel: string; mode?: "delta" | "safety" | "wins"; displayValue?: string | null };
...
export default function KpiCard({ eyebrow, value, previousValue, previousLabel, mode = "delta", displayValue }: Props) {
  const formula = eyebrow.includes("Formula");
  const comparison = value !== null && previousValue !== null ? value - previousValue : null;
  const display = value === null ? "—" : displayValue ?? (mode === "safety" ? value.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : mode === "wins" ? value.toLocaleString("pt-BR") : signed(value));
  return <article className={`kpi-card ${formula ? "formula" : "sports"}`}>
    ...
    <div className="kpi-description">{mode === "safety" ? `Safety Rating atual • ${previousLabel}: ${previousValue?.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) ?? "—"}` : mode === "wins" ? `Vitórias oficiais • ${previousLabel}: ${previousValue?.toLocaleString("pt-BR") ?? "—"}` : "Variação acumulada na season"}</div>
  </article>;
}
