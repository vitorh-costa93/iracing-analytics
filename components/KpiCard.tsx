type Props = { eyebrow: string; value: number | null; previousValue: number | null; previousLabel: string; mode?: "delta" | "safety"; displayValue?: string | null };
...
export default function KpiCard({ eyebrow, value, previousValue, previousLabel, mode = "delta", displayValue }: Props) {
  ...
  const display = value === null ? "—" : displayValue ?? (mode === "safety" ? value.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : signed(value));
  return <article className={`kpi-card ${formula ? "formula" : "sports"}`}>
    ...
    <div className="kpi-description">{mode === "safety" ? `Safety Rating atual • ${previousLabel}: ${previousValue?.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) ?? "—"}` : "Variação acumulada na season"}</div>
  </article>;
}
