...
export async function GET(request:NextRequest){
 try{
  ...
  const sections=await Promise.all((segments.length?segments:[SEGMENTS[0]]).map(async segment=>{
   ...
   // 08/09/2026: curva a curva só compara o que existe nos dois períodos -- sem isso, "as 3
   // combinações carro+pista com mais voltas" escolhidas independentemente em cada período quase
   // nunca coincidem (o calendário muda de pista entre seasons).
   ...
   // 08/09/2026: "por que fiquei mais distante da melhor volta?" -- ponto de frenagem por curva
