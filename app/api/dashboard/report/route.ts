...
export async function GET(request:NextRequest){
 try{
  ...
  const rows=await racesFor(driver.id,previousStart,new Date(new Date(currentStart).getTime()+84*86400000).toISOString()),currentRows=rows.filter(x=>new Date(x.raced_at)>=new Date(currentStart)),previousRows=rows.filter(x=>new Date(x.raced_at)<new Date(currentStart)),currentWeek=currentRows.reduce<number|null>((latest,row)=>row.season_week!==null&&(latest===null||row.season_week>latest)?row.season_week:latest,null);
  const sections=await Promise.all(SEGMENTS.map(async segment=>{
   const now=currentRows.filter(row=>row.category===segment.category&&segment.match(row)),before=previousRows.filter(row=>row.category===segment.category&&segment.match(row)),selected=scope==="week"&&currentWeek!==null?now.filter(row=>row.season_week===currentWeek):now,baseline=scope==="week"&&currentWeek!==null?now.filter(row=>row.season_week!==currentWeek):before;
   const[retirements,retirementReference,inputs,inputReference]=await Promise.all([retirementEvents(driver.id,selected,current.season_name),retirementEvents(driver.id,baseline,scope==="week"?current.season_name:previous.season_name),telemetryInputProfile(driver.id,segment.category,selected,current.season_name,scope==="week"?currentWeek:null),telemetryInputProfile(driver.id,segment.category,baseline,scope==="week"?current.season_name:previous.season_name,null)]);
   const base=buildEngineerSection(segment.category,selected,baseline,scope,scope==="week"?currentWeek:null,segment.label,segment.id),weekAverage=baseline.length?total(baseline.map(row=>row.irating_after-row.irating_before))/baseline.length:null;
   return{...base,comparison:scope==="week"?"Referência: média das outras weeks desta mesma season = "+(weekAverage===null?"—":String(weekAverage>0?"+":"")+weekAverage.toFixed(1)+" de iRating por corrida")+" em "+String(baseline.length)+" corridas.":base.comparison,retirements,retirementReference,retirementComparison:{currentCount:retirements.length,referenceCount:retirementReference.length,currentRate:selected.length?Number((retirements.length/selected.length*100).toFixed(1)):null,referenceRate:baseline.length?Number((retirementReference.length/baseline.length*100).toFixed(1)):null},telemetryInputs:inputs,telemetryInputReference:inputReference,seasonComparison:scope==="season"?compareSeasons(now,before):null};
  }));
