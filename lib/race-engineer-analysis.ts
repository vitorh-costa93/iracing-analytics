...
export function buildEngineerSection(category:Category,rows:RaceInput[],previous:RaceInput[],scope:"week"|"season",week:number|null,sectionLabel?:string,segment?:string){
 ...
 const averageGain=avg(races.filter(row=>delta(row)>0).map(delta))??0,averageSevereLoss=severe.length?Math.abs(avg(severe.map(delta))??0):null,gainsToRecover=averageGain&&averageSevereLoss!==null?averageSevereLoss/averageGain:null;
