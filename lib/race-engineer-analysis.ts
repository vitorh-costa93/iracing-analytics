...
function executiveSummary(opts:{scope:"week"|"season";races:number;baselineRaces:number;net:number;previousNet:number;wins:number;previousWins:number;severeCount:number;severeShare:number;worstRun:{length:number;delta:number}|undefined;incidentsNow:number|null;incidentsBefore:number|null}):string{
 ...
 // 09/09/2026: "cometi erros de tomada de decisão... me envolvi em muitas confusões que me fizeram
 // perder iRating" -- incidentes por corrida é o único dado que aproxima isso (não dá pra isolar
 // contato de erro de pilotagem com o que a base guarda, ver nota em incidentSummary), mas uma piora
 // real na média já é evidência concreta o suficiente pra citar, sem inflar em causa confirmada.
 const incidentDelta=incidentsNow!==null&&incidentsBefore!==null?incidentsNow-incidentsBefore:null,incidentsWorse=incidentDelta!==null&&incidentDelta>=1;
 const incidentsText=incidentsWorse?" Você também se envolveu em mais incidentes por corrida: "+incidentsNow!.toFixed(1)+" contra "+incidentsBefore!.toFixed(1)+" na referência.":"";
 return opening+driver+incidentsText;
}
...
export function buildEngineerSection(category:Category,rows:RaceInput[],previous:RaceInput[],scope:"week"|"season",week:number|null,sectionLabel?:string,segment?:string){
 ...
 // 09/09/2026: "consigo identificar o incidente de tipo contato... 4 pontos junto significa contato
 // com dano" -- checado direto no banco: race_results.incidents é só o TOTAL por corrida (visto
 // 0-19+), sem quebra por evento; o Garage61 (laps.garage61_payload) também não guarda pontos de
 // incidente por volta. Não dá pra confirmar "este incidente específico foi contato" com o que
 // temos. O proxy mais honesto: contar corridas com 4+ pontos de incidente no total (consistente
 // com pelo menos um contato, mas pode ser vários offs pequenos somados) -- rotulado como tal, nunca
 // como confirmação.
 const HIGH_INCIDENT_THRESHOLD=4;
 const incidentStats=(rowsIn:RaceInput[])=>{const withIncidents=rowsIn.filter(row=>typeof row.incidents==="number"),total=sum(withIncidents.map(row=>row.incidents as number)),highCount=withIncidents.filter(row=>(row.incidents as number)>=HIGH_INCIDENT_THRESHOLD).length;return{races:withIncidents.length,total,average:round(withIncidents.length?total/withIncidents.length:null),highCount,highRate:withIncidents.length?round(highCount/withIncidents.length*100):null}};
 const incidentSummary={current:incidentStats(races),reference:incidentStats(baseline)};
