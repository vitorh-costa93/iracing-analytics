...
export async function retirementEvents(driverId:string,races:RaceInput[],seasonName:string){
 ...
 const confirmedByDriver=races.find(r=>Math.abs(new Date(r.raced_at).getTime()-new Date("2026-09-04T18:30:00.000Z").getTime())<=30*60000&&key(r.car_name).includes("sf23")&&key(r.track_name).includes("silverstone"));if(confirmedByDriver)events.push({racedAt:confirmedByDriver.raced_at,context:confirmedByDriver.car_name+" • "+confirmedByDriver.track_name,delta:confirmedByDriver.irating_after-confirmedByDriver.irating_before,positionChange:confirmedByDriver.position_change,incidents:confirmedByDriver.incidents,type:"Tow / abandono confirmado pelo piloto",confidence:"confirmed",laps:confirmedByDriver.finish_position});
 const unique=new Map<string,typeof events[number]>();for(const event of events){const eventKey=event.racedAt+"|"+event.context;const old=unique.get(eventKey);const priority=(x:typeof event)=>x.confidence==="confirmed"?3:x.type.includes("pit +")?2:1;if(!old||priority(event)>priority(old))unique.set(eventKey,event)}return [...unique.values()].sort((a,b)=>a.confidence!==b.confidence?(a.confidence==="confirmed"?-1:1):a.delta-b.delta);
}
