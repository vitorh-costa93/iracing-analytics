...
export async function telemetryInputProfile(driverId:string,category:Category,races:RaceInput[],seasonName:string,week:number|null){
 ...
 const {data,error}=await supabaseAdmin.from("laps").select("id,car_id,track_id,lap_time,clean,off_track,incomplete,discontinuity,telemetry_path,garage61_payload").eq("driver_id",driverId).in("car_id",ids).contains("garage61_payload",{season:{name:seasonName}}).not("telemetry_path","is",null).order("synced_at",{ascending:false}).limit(500);if(error)throw error;
 const raceTimes=races.map(r=>new Date(r.raced_at).getTime()).filter(Number.isFinite);
 const candidates=((data??[]) as Lap[]).filter(l=>{const p=l.garage61_payload??{},time=p.startTime?new Date(p.startTime).getTime():NaN;return p.sessionType===3&&l.clean===true&&l.off_track!==true&&l.incomplete!==true&&l.discontinuity!==true&&typeof l.lap_time==="number"&&l.lap_time>0&&Number.isFinite(time)&&raceTimes.some(race=>Math.abs(time-race)<=2*3600000)}).slice(0,24);
 const rawSamples:Array<Omit<LapSample,"gap">>=[];
 for(const lap of candidates){if(!lap.telemetry_path)continue;const d=await supabaseAdmin.storage.from("telemetry").download(lap.telemetry_path);if(d.error||!d.data)continue;let raw=Buffer.from(await d.data.arrayBuffer());try{if(lap.telemetry_path.endsWith(".gz"))raw=gunzipSync(raw)}catch{continue}const lines=raw.toString("utf8").split(/\r?\n/);if(lines.length<3)continue;const headers=lines[0].split(",").map(norm),throttle=values(lines,index(headers,["throttle"])),brake=values(lines,index(headers,["brake"])),steering=values(lines,index(headers,["steeringwheelangle","steering"]));const ts=Math.max(...throttle.map(Math.abs),0)>1.5?100:1,bs=Math.max(...brake.map(Math.abs),0)>1.5?100:1,p=lap.garage61_payload??{};rawSamples.push({lapTime:lap.lap_time!,session:String(lap.car_id)+"|"+String(lap.track_id)+"|"+(p.startTime??""),throttleSmoothness:smoothness(throttle.map(x=>x/ts)),brakeSmoothness:smoothness(brake.map(x=>x/bs)),steeringSmoothness:smoothness(steering)})}
 const bestBySession=new Map<string,number>();for(const sample of rawSamples)bestBySession.set(sample.session,Math.min(bestBySession.get(sample.session)??Infinity,sample.lapTime));
 const samples:LapSample[]=rawSamples.map(sample=>({...sample,gap:sample.lapTime-(bestBySession.get(sample.session)??sample.lapTime)})),gaps=samples.map(s=>s.gap),lapStd=sd(gaps),gapMean=avg(gaps),contexts=bestBySession.size,onlyOne=contexts===1,lapTimes=samples.map(s=>s.lapTime),throttle=metric(samples,"throttleSmoothness"),brake=metric(samples,"brakeSmoothness"),steering=metric(samples,"steeringSmoothness");
 return{files:samples.length,laps:samples.length,coverage:String(contexts)+" sessões de corrida; tempos normalizados pela melhor volta de cada sessão",meanLapSeconds:onlyOne?round(avg(lapTimes),3):null,bestLapSeconds:onlyOne&&lapTimes.length?round(Math.min(...lapTimes),3):null,averageGapToBestSeconds:round(gapMean,3),lapStdDevSeconds:round(lapStd,3),lapCvPct:null,throttleRepeatabilityPct:repeatability(throttle),brakeRepeatabilityPct:repeatability(brake),steeringRepeatabilityPct:repeatability(steering),throttleLapCorrelation:correlate(throttle),brakeLapCorrelation:correlate(brake),steeringLapCorrelation:correlate(steering),note:samples.length>=8?"Os tempos de pistas diferentes foram normalizados pelo gap para a melhor volta limpa da mesma sessão. A correlação mostra se variar um input acompanhou aumento ou redução desse gap.":"Amostra pequena: use os valores como sinal, não como conclusão."};
}
