...
export async function telemetryInputProfile(driverId:string,category:Category,races:RaceInput[],seasonName:string,week:number|null){
 ...
 const counts=new Map<string,number>();for(const sample of rawSamples)counts.set(sample.session,(counts.get(sample.session)??0)+1);const comparableRaw=rawSamples.filter(sample=>(counts.get(sample.session)??0)>=2);if(!comparableRaw.length)return empty("Há telemetria neste período, mas ainda não existem duas voltas limpas do mesmo carro e pista para calcular consistência.");
