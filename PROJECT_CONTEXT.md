...
Os `.sto` possuem uma seção criptografada que não pode ser interpretada diretamente pela aplicação. Com autorização explícita do usuário na própria tela, os dois arquivos selecionados são enviados server-side ao endpoint de decodificação do SetupDelta. Os parâmetros retornados, nome do decodificador, horário da decodificação e horário do consentimento ficam cacheados em `setup_files`, evitando retransmissões nas comparações seguintes. O arquivo original permanece no bucket privado e não é exposto ao browser. O consentimento precisa ser renovado na UI para iniciar cada comparação, embora arquivos já decodificados usem somente o cache. O recurso depende da disponibilidade e da cobertura de carros do serviço externo; erros de formato ou carro não suportado devem ser mostrados ao usuário sem fabricar diferenças.

O Garage61 pode indicar `canViewSetup`, mas o endpoint observado não entrega o binário do setup. Portanto, enquanto não existir um endpoint autorizado para download, o usuário ainda precisa enviar manualmente pelo menos o fixed e o comercial uma vez. O upload registra `setup_kind` e aceita fixed, commercial, open ou unknown; sobrescrever um arquivo invalida seu cache decodificado.

Em 20/08/2026, a biblioteca local do PC foi inventariada: existem 2.941 `.sto` no total, dos quais 282 pertencem à pasta `26S3`. O importador versionado adiciona somente esses arquivos e o `-Current-` dos sete carros envolvidos, totalizando 289 objetos no bucket privado `private-setups`, além de um manifesto sem conteúdo decodificado. A UI do Setup Lab mostra o catálogo agrupado por carro, fornecedor e pista inferida. Setups comerciais nunca devem ser publicados nem enviados a um decodificador sem consentimento explícito. Os setups padrão internos do iRacing não existem como arquivos soltos na instalação; ficam empacotados no simulador. Quando necessário, o usuário deve salvar/exportar o fixed pelo Garage para produzir um `.sto`/HTML utilizável.

O endpoint server-side anteriormente usado no SetupDelta passou a responder HTTP 410. O comparador agora traduz esse estado em orientação clara e preserva os arquivos no cofre, sem alegar uma comparação inexistente. A evolução preferida é aceitar o HTML exportado pelo Garage e comparar parâmetros localmente; não depender novamente de um endpoint externo não documentado sem contrato e monitoramento.
