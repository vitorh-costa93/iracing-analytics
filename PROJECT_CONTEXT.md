...
- O Telemetry Lab expõe o critério da volta representativa: melhor volta de corrida quando existe, melhor Practice para preparação quando ainda não existe corrida e, para SF23, corrida sem P2P/Overtake. A explicação deve mostrar tipos de sessão encontrados, sem prometer uma certeza que os canais não suportem.
- O Race Debrief é progressivo: resumo, pontos fortes e foco de treino aparecem primeiro; ritmo, inputs, outliers, curva a curva e volta ideal/consistência por setor são evidência expansível. A UI sinaliza amostra robusta somente com pelo menos 10 voltas analisadas; abaixo disso, usa a etiqueta de amostra limitada.

## Integridade de mapas de Le Mans (10/09/2026)
...
- 23/09/2026 — segundo incidente de "Cached Egress" (9,1 GB / 5 GB no ciclo 09/09–09/10). Causa: `listSeasonsAndTracks` em `app/api/telemetry/car-comparison/route.ts` baixava até 3 arquivos de telemetria por carro por pista elegível a CADA carregamento do seletor de pistas, só para checar cobertura de GPS, sem guardar o resultado (~244 MB de arquivos distintos servidos ~37×). Correção: cache permanente por volta em `lap_gps_checks` (`lib/lap-gps-check-cache.ts`); só volta sem cache baixa arquivo. Regra: nenhuma rota de listagem/seletor pode baixar telemetria sem cache persistente por `lap_id`.
- 23-24/09/2026 — teto rígido de download de telemetria: toda leitura do Storage passa por `lib/telemetry-storage.ts` (100 MB/dia, contador em `telemetry_download_usage`; acima disso a leitura é recusada). Também descomprime `.csv.gz` em todas as rotas (debrief, baselines, visualizador e comparação liam o gzip como texto e quebravam) e grava fallbacks do Garage61 sempre como `.csv.gz` (antes regravavam `.csv` e alimentavam um ciclo de recompressão). A compactação agora parte de `laps.telemetry_path` (só voltas em uso) e usa upsert. Conferir consumo: `select * from telemetry_download_usage order by day desc`.
- Pendente (precisa de autorização do piloto): ~210 objetos `.csv` órfãos no bucket `telemetry` (nenhuma volta aponta para eles). Não geram tráfego, só ocupam espaço.
- Build intermitente: `next/font/google` (Big Shoulders) falhou 2 vezes na Vercel em 23-24/09 ("Failed to find font override values"); um redeploy resolve. Se repetir, trocar por `next/font/local`.

## Redesign Night Grid — etapa 1: base visual (25/09/2026, branch `redesign-ui-b`)

- Plano e decisões fixas: `docs/redesign-plan.md`; referência visual: `docs/redesign-mockup/*.dc.html`.
- Tokens em `app/night-grid.css` com prefixo `--ng-` (paleta do plano). Os tokens antigos de `app/globals.css` ficam intactos até as telas antigas serem migradas; depois os dois conjuntos podem ser unificados. O tema claro (`ThemeToggle`, `data-theme="light"`) tem uma tradução provisória dos tokens `--ng-`, não desenhada no mockup.
- Fontes via `next/font/google`: Chakra Petch (`--font-chakra`, títulos e números) e IBM Plex Sans (`--font-plex`, texto). `body` usa números tabulares por padrão.
- Cabeçalho global fixo `components/AppHeader.tsx` (em `app/layout.tsx`): Visão Geral `/`, Telemetry Lab `/telemetry`, Debriefs `/debriefs` (placeholder até a etapa 5), Setup Lab `/setup`; frescura das fontes e identidade do piloto vêm de `/api/sync/status` (agora devolve também `driver` com uma leitura de uma linha de `drivers`), com uma única requisição por página compartilhada com `DataFreshness` (`lib/use-sync-status.ts`).
- "Atualizar dados" saiu do cabeçalho antigo da Visão Geral para o cabeçalho global. A sequência (sync/all → sync/telemetry → sync/rating-history) foi extraída sem mudanças para `lib/data-sync-action.ts` e anuncia progresso/fim por eventos de janela; a Visão Geral escuta e recarrega o dashboard.
- `AppTabs` ficou só com o botão "Favoritos" (bookmarklets); a navegação entre áreas é do cabeçalho.
- Componentes base em `components/ui/`: `Panel`/`PageTitle`, `SegmentedControl`, `SelectPill` (select nativo), `Chip` (tag/solid/pill/code), `KpiCard` (barrinha vertical de categoria dentro do cartão, minigráficos lines/steps/bars com a geometria do mockup em `lib/sparkline.ts`) e `CategoryHeading`.
- Ambiente local: `.env.local` tem `NEXT_PUBLIC_SUPABASE_URL` e a chave de serviço como placeholders, então `npm run build` só coleta os dados das rotas com `NEXT_PUBLIC_SUPABASE_URL` vindo de `SUPABASE_URL` no ambiente do processo, e o dev server local não lê o Supabase (as rotas respondem erro de chave). Não é regressão desta etapa.

## Redesign Night Grid — etapa 3: Telemetry Lab · semana ativa (26/09/2026)

- Tela em `components/ActiveWeekTelemetry.tsx` (orquestra dados) + `components/telemetry/` (`LapMap`, `RepresentativeLapChart`, `CornerByCorner`, `SectionPopup`); CSS só em `app/night-grid-telemetry.css` (`--ngt-`, classes `.ngt-`). `app/telemetry/page.tsx` usa `ng-page`; título e seletor Semana ativa / Race Debrief / Comparação de carros em `TelemetryTabs` (as duas últimas ainda com a UI antiga até a etapa 4). A frescura e o "Atualizar dados" ficam só no cabeçalho global.
- Parsing de CSV/IBT e interpolação saíram sem mudança para `lib/telemetry-trace.ts` (+ `hasCompleteGps`: GPS em 90% das amostras, do início ao fim, sem buraco > 3% da volta; o subtítulo só diz "traçado completo no GPS" quando isso vale).
- Análise em `lib/lap-analysis.ts`: mesma base de tempo de antes (grade de 0,25%, 1/velocidade normalizada pelo tempo da volta), agora com TODOS os trechos de curva, tempo perdido de cada curva dentro da sequência (as partes somam o total) e métricas em unidades de pilotagem (ponto de freio em metros, velocidade mínima, volta ao acelerador em segundos, saída, % de pedal, volante, marcha, traçado). O lado esquerda/direita do traçado usa o produto vetorial correto (o sinal antigo estava invertido) e só aparece quando 80% das amostras concordam e a distância passa de 1,5 m.
- Sequências em `lib/corner-sequences.ts`: curvas separadas por menos de 120 m (1,5 × `ABSOLUTE_MERGE_GAP_METERS`, teto de 2,5% da volta) E sem que você e a referência voltem a acelerar tudo (≥ 95% por 50 m) entre os ápices. Só a proximidade encadeava demais: a detecção por heading devolve, por exemplo, Road Atlanta 1-8 como um complexo contínuo de 1,2 km. Cada trecho inclui a zona de frenagem (150 m antes) e 40 m de saída, sem sobrepor o vizinho.
- Textos em `lib/engineer-talk.ts` (voz de engenheiro: décimos/centésimos, metros, km/h, % de pedal; sem "p.p.", "Δ", "--"). A causa-raiz citada e o conselho seguem a ordem das fases da curva (freio → meio → acelerador → saída). Em sequência, o texto diz quem paga e quem ganha (sacrifício que compensa, que não compensa, ou entrada forte que custa a saída).
- Cartões de contexto: os 3 primeiros por (tem volta, teve corrida, sessões, voltas); os demais num seletor "N contextos com atividade". O gap dos cartões não abertos vem só de cache local (`localStorage`, chave `ngt-context-gap:v1`), sem baixar telemetria extra. `/api/telemetry/active-week` passou a devolver `car.category` (sports/formula) e `car.carClass` (GT3/GTP/LMP2 por `car_groups`, com reforço por nome para LMP2), consultas pequenas e não fatais.
- Saíram desta tela: gráfico de 10 canais, relatório de inputs/microcorreções (volta na etapa 4 com o Race Debrief/Comparação). A consistência por setor já não estava aqui (continua no Race Debrief).
- Observado na validação local: nenhuma volta representativa desta semana tinha `telemetry_path`, e o fallback ao vivo do Garage61 respondeu 401 no ambiente local; a interface foi testada com a telemetria armazenada de outra volta real da mesma pista (troca feita só no navegador).

## Redesign Night Grid — etapa 4: Race Debrief e Comparação de carros (26/09/2026)

- Race Debrief novo: `components/telemetry/RaceDebriefView.tsx` + `/api/telemetry/race-debrief` (Supabase-first, sem chamada ao Garage61; volta sem `telemetry_path` não é baixada ao vivo). Seletor das últimas 40 corridas (race_results) casadas com a sessão de corrida (session_type 3); só voltas `sessionType = 3` do evento. Cache em `race_debriefs` com `rating_category = "race:<irstats_race_id>"` (CACHE_VERSION, invalidado quando muda o conjunto de voltas com telemetria; no máximo 30 linhas; só grava em produção). As linhas antigas por categoria do Meu Debrief não são tocadas.
- "Curva a curva × Consistência": `lib/self-consistency.ts` (trechos/sequências de `lib/corner-sequences.ts`; variação de freio em m, acelerador em s, volante em °; terço mais rápido contra o mais lento; ganho de repetir; acerto) e textos em `lib/debrief-talk.ts`. Degrada com < 3 voltas com telemetria (só aviso) e < 5 (sem rápidas × lentas).
- Microcorreções: `lib/microcorrections.ts` (inversão do volante de pelo menos 4°, movimento de até 0,5 s, acima de 60 km/h, reamostrado a 20 Hz; por minuto no debrief, por volta na comparação). Não confundir com as "correções" relativas de `lib/traction-events.ts`, que continuam existindo.
- `lib/sector-report.ts` (extraído da rota sectors) agora filtra `sessionType = 3`: o evento Garage61 junta classificação e corrida.
- Comparação de carros: `components/telemetry/CarCompareView.tsx`; a rota `car-comparison` ganhou `avgLapSeconds`, `microcorrections`, `fastestLap` (id + distâncias das microcorreções), `ownCarId`, `plausibleLapCount` e `compact=1` (sem curvas/GPS por setor). Curva a curva = `compareLaps` da semana ativa contra o carro escolhido; popup da etapa 3 reaproveitado (`refNoun`/`refLabel`; `lib/engineer-talk.ts` aceita `carNoun()`).
- Limitação de dados observada: as corridas desde 27/08 não têm telemetria armazenada (`telemetry_path` nulo); o debrief mostra resultado, ritmo e setores e avisa que falta telemetria.
...
## Redesign Night Grid — etapa 6: Setup Lab (26/09/2026, branch `redesign-ui-b`)

- `/setup` usa `components/setup/` (`SetupLabView`, `EngineerChat`, `ProposalPanel`, `SetupComparator`) e `app/night-grid-setup.css`; `components/SetupLab.tsx` foi removido (só era usado por `/setup`). Referência: `docs/redesign-mockup/Setup.dc.html`.
- Topo: carro+pista da season e **setup ativo**. A conversa (`engineer_conversations`, uma por piloto/season/carro/pista, com carregar e "Nova conversa") é sempre sobre o setup ativo. Digitar `/` abre o menu de setups do carro (`/setup Nome`) e os citados aparecem como chip `@ Nome`; no máximo 2 citações entram no prompt, como diferença real contra o ativo. Os chips de sintoma só preenchem o campo.
- Turnos gravados ganharam campos opcionais: no piloto `setupId`, `setupName`, `onProposal`, `mentions[{id,name}]`; no engenheiro `proposal`. Regenerar reaproveita o setup e as citações gravados no turno do piloto.
- Proposta estruturada (`lib/engineer-proposal.ts`): o prompt numera os parâmetros do setup ativo (p1, p2...) e pede, só quando houver mudança concreta, um bloco final `<proposta>{json}</proposta>` com `id`, direção e passos. O servidor separa o bloco do texto, valida e resolve A → B antes de gravar; bloco ausente ou inválido vira "sem proposta ainda" e nunca quebra o chat. B só ganha número para inteiros sem unidade física (posições, cliques); valores contínuos (N/mm, %, graus) aparecem como "↑ 1 passo". Com "B · ... (proposta)" escolhido como setup ativo, a próxima proposta acumula sobre a anterior. Nenhum `.sto` é reescrito.
- "Comparar dois setups" segue em `/api/setup/compare` (diff determinístico) e agora devolve `explanation` de `lib/setup-explain.ts`: frase-resumo, "como você vai sentir" por fase, "quando usar cada um", passos de teste específicos e glossário só com os termos usados. Sem LLM e sem custo. Os nomes falados são encurtados (`shortPairNames`: "Quali" × "Race"). Leituras da sessão (pressão quente, pneu restante, folga da mola central) ficam fora da análise porque não são ajuste.
- Textos de `lib/setup-diff.ts` e o prompt (`lib/engineer-prompt.ts`) reescritos no tom de engenheiro de pista: sem "Setup A/B" genérico, sem "aba • seção", sem "--". Corrigidos: "Brake Pressure Bias" caía em pressão de pneu e Splitter não contava como aerodinâmica.
- O que o mockup não redesenha ficou no rodapé recolhível "Biblioteca local, anexar setup e importação": biblioteca local privada, envio de `.sto` para o cofre e os favoritos de importação. Copiar e Regenerar ficam sob cada resposta do engenheiro.
