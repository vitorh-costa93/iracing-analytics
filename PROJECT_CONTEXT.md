...
- Pendente: 482 de 661 arquivos do bucket `telemetry` ainda estão em CSV puro (~804 KB); só 179 estão em `.csv.gz` (~254 KB). A leitura de `.gz` só existe em `lib/telemetry-input-profile.ts` — car-comparison, debrief, baselines e garage61 leem `file.text()` direto. Comprimir o restante (~3× menos egress) exige antes tornar essas leituras gz-safe.
